...
Current preview/standard/research settings are 12/24/50 Adam iterations, 16/16/32 optimization spp, 64/128/256 validation spp, and 64/128/256 maximum render sides. Ultra uses 75 iterations, at most 32 optimization spp, 256 validation spp, and a 1024-pixel maximum render side. A memory guard reduces stochastic optimization to as little as 1 spp so one pass contains no more than about eight million pixel samples. A separate stability guard caps the Ultra learning rate at 2% of normalized fitting-grid pixel pitch. Both effective values and whether the crop was downsampled are recorded in `result.json`. It optimizes absolute vertex heights rather than a bounded correction. `research` is a CLI identifier, not an accuracy certification, and Ultra is not an accuracy certification either. Before/after checks use higher-sample renders, and provisionally accepted candidates receive a second-seed withheld-view check. Check convergence and angular/physical source-size sensitivity empirically; these budgets are not prescribed by the projective-sampling paper.

On 2026-09-15, Windows Error Reporting identified a production Ultra failure as exception `0xc0000094` at `drjit-core.dll` offset `0x4a71a`. Disassembly and the exact Dr.Jit-Core commit used by Dr.Jit 1.3.1 map that instruction to `CUDAThreadState::block_reduce()`: `round_pow2(block_size)` overflows to zero when a monolithic reduction exceeds `2^31` entries, and the following `thread_count / chunk_size` performs an integer division by zero. Hierarchical 1,048,576-element loss sums fixed the worker's explicit reduction but a second production run with the verified 0.2.12 worker reproduced the same exception and offset, proving that reverse-mode rendering also invokes the vulnerable native path internally. Version 0.2.13 therefore source-builds Dr.Jit Core commit `213983e47c99db0c6ab5e3dfce952e68bb9a8bd3` with overflow-safe ceiling division and direct selection of the existing 1024-wide kernel for block and prefix reductions of 1024 or more. The installer bundles the exact patch, records its source commit, patch hash, and DLL hash, and Ultra CUDA verifies the marker against the actual DLL before processing. Worker-level hierarchical value/gradient tests and upstream Dr.Jit CUDA reduction tests pass. This addresses the identified division-by-zero path without removing image terms or visibility derivatives; unrelated allocation and renderer limits remain.

The first production timing trial after that crash fix saturated a 24 GB CUDA device and did not finish the first Ultra training-light pass within 11 minutes. Because each of 75 iterations evaluates six differentiable basis passes per training light and periodic 256-spp material checkpoints, the uncapped native grid was not operationally viable. Version 0.2.14 caps Ultra's longest optimization side at 1024 pixels while retaining the absolute-height parameterization, all triangle-supported fitting-grid vertices, all supported training lights, and both visibility-derivative streams. `result.json` records the source crop, actual grid, cap, and whether downsampling occurred. This is an explicit workload limit, not evidence that 1024 pixels is sufficient for scientific accuracy.

The first larger-than-grid 0.2.14 run completed all 75 optimization iterations but failed while expanding the selected candidate: a 768-row fitting mask was applied directly to the 1920-row source crop. Version 0.2.15 nearest-resamples the boolean fitting support to the source crop, intersects it with the original geometry mask, and only then copies the bilinearly expanded absolute height. A mismatched-grid regression test checks the candidate, unchanged unsupported baseline, signed difference, and outside-mask zeros. Repeated indirect-silhouette octree warnings in that run preceded but did not cause the Python `IndexError`; they indicate weak or absent indirect visibility guidance and make progressive surface softening scientifically concerning rather than evidence of convergence.

The same 1024-capped run exposed a live-preview integration defect: its side-by-side normal and hillshade frame could be 2048 by 768 pixels, while the C++ GUI intentionally refuses to decode a progress image larger than 1024 by 512. Version 0.2.15 proportionally downsamples only this diagnostic frame before publishing it. The optimization grid and reconstruction products are unchanged. A numerical regression checks the maximum dimensions, aspect ratio, range, finite values, and no-upscaling behavior.

Following the production evaluation, version 0.2.16 no longer offers Ultra in the GUI quality selector. Preview, Standard, and High detail remain visible. The Ultra enum, CLI parser, serialized-project reader, backend implementation, and numerical/rendering tests remain in place for controlled experiments; a stored Ultra selection is deliberately presented as Standard when opened in the GUI. A GUI lifecycle regression checks the three visible labels and this fallback.

The targeted `cuda_ad_rgb` Ultra recovery gate on 2026-09-14 used the same 24 x 24 matched-model fixture, an absolute flat start, 40 iterations, 8 optimization spp, and 32 validation spp. It completed the streamed native branch, accepted the candidate, changed the solution, improved training loss by 1.47% and withheld loss by 3.57%, reduced mean height-normal error from about 9.18 to 8.90 degrees, and reduced offset-normalized height RMSE from about 0.989 to 0.980 pixels. The truth-start Ultra gate still selected the byte-identical baseline. This is a small functional/stability regression, not evidence for full-size memory sufficiency, production-budget convergence, independent-model accuracy, or dark glossy recovery.
