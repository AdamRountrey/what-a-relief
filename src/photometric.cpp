...
double weightedLightConditionNumber(
    ...
    for (size_t i = 0; i < lights.size(); ++i) {
        ...
        // Preserve near-field attenuation: condition the same rows as the fit.
        const cv::Vec3d direction(lights[i]);
        for (int row = 0; row < 3; ++row) {
            for (int col = 0; col < 3; ++col) {
...
void classifyRobustObservations(
    ...
    const cv::Vec3d normal = solution / cv::norm(solution);
    scratch.weights = scratch.fit.reliability;
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        if (!std::isfinite(observed)) continue;
        const double linearPrediction = cv::Vec3d(lights[i]).dot(solution);
        ...
        const double predictionVariance = std::max(0.0, light.dot(scratch.fit.covariance * light));
        const double shadowSigma = std::sqrt(sigma * sigma + predictionVariance);
        const double difference = observed - predicted;
        scratch.standardizedResiduals[i] = difference / sigma;
        if (definiteSaturation[i]) {
            scratch.classes[i] = RobustObservationClass::Saturated;
            continue;
        }
        const double significantDeviation = std::max({0.012, 0.12 * predicted, 3.5 * sigma});
        const double significantShadow = std::max({0.012, 0.12 * predicted, 3.5 * shadowSigma});
        if (observed <= shadowThreshold) {
            if (linearPrediction < -0.5 * std::sqrt(predictionVariance) ||
                (-difference > significantShadow && observed < 0.55 * predicted)) {
                scratch.classes[i] = RobustObservationClass::Shadow;
            }
            continue;
        }
        // Labels are diagnostics only. Neither half-vector agreement nor the
        // discontinuous label thresholds modify the fitted normal or weights.
        if (-difference > significantShadow && observed < 0.55 * predicted) {
            scratch.classes[i] = RobustObservationClass::Shadow;
        } else if (difference > significantDeviation && halfVectorAlignment(normal, lights[i], viewDirection) > 0.90) {
            scratch.classes[i] = RobustObservationClass::Highlight;
        } else if (scratch.weights[i] < 0.25 && std::abs(difference) > significantDeviation) {
...
bool solveRobustPixel(
    ...
    cv::Vec3d solution;
    if (!robust_fit::fit(lights, intensities, definiteSaturation, shadowThreshold,
            probableSaturationThreshold, scratch.fit, solution,
            scratch.headroom.empty() ? nullptr : &scratch.headroom)) return false;
    const double intensityScale = std::max(0.02, scratch.fit.signalReference);
    classifyRobustObservations(solution, lights, intensities, definiteSaturation,
        shadowThreshold, intensityScale, viewDirection, scratch);

    result.localCondition = weightedLightConditionNumber(lights, scratch.fit.coefficients);
    ...
    for (size_t i = 0; i < intensities.size(); ++i) {
        ...
        if (scratch.weights[i] > 0.05 && observed > shadowThreshold && predicted > 0.0) {
            ++result.inlierCount;
        }
...
void solvePhotometricStereo(
    ...
    if (!headroomWeights.empty()) {
        if (headroomWeights.size() != images.size()) die("Sensor reliability count must match image count.");
        for (const cv::Mat& weights : headroomWeights) {
            if (!weights.empty() && (weights.size() != images.front().size() || weights.type() != CV_8U)) {
                die("Sensor reliability maps must be matching 8-bit images or empty (unit reliability).");
            }
        }
    }
    ...
    if (solverMode == NormalSolverMode::Robust) {
        ...
        cv::parallel_for_(cv::Range(0, rows), [&](const cv::Range& rowRange) {
            ...
            RobustScratch scratch;
            if (!headroomWeights.empty()) scratch.headroom.resize(images.size());
            ...
            for (int y = rowRange.start; y < rowRange.end; ++y) {
                ...
                for (int x = 0; x < cols; ++x) {
                    ...
                    for (int i = 0; i < n; ++i) {
                        ...
                        if (!headroomWeights.empty()) {
                            scratch.headroom[i] = headroomWeights[i].empty() ? 1.0f :
                                headroomWeights[i].ptr<uchar>(y)[x] / 255.0f;
                        }
                    ...
                    iterationsByRow[y] += static_cast<std::uint64_t>(scratch.fit.iterations);
                    limitedByRow[y] += scratch.fit.converged ? 0 : 1;
        ...
        if (solvedPixels > 0) {
            diagnostics.robustMeanIterations = static_cast<double>(
                std::accumulate(iterationsByRow.begin(), iterationsByRow.end(), std::uint64_t(0))) / solvedPixels;
            diagnostics.robustNonconvergedFraction = static_cast<double>(
                std::accumulate(limitedByRow.begin(), limitedByRow.end(), std::uint64_t(0))) / solvedPixels;
        }
