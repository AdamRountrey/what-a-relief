...
void writePrintablePlyMesh(
    ...
    if (!heightSource.empty()) {
        out << "comment height source " << heightSource << "\n";
    }
...
cv::Mat albedoPreview8U(const cv::Mat& albedo, const cv::Mat& mask) {
    return normalizeFloatTo8U(albedo, mask, true);
}
...
void saveGeometryMeshes(
    ...
    if (height.empty() || height.type() != CV_32F || geometryMask.type() != CV_8U ||
        geometryMask.size() != height.size() ||
        (!vertexColor.empty() && (vertexColor.type() != CV_8U || vertexColor.size() != height.size()))) {
        die("Mesh export requires matching float height, byte mask, and optional byte albedo preview.");
    }
    if (heightSource.find_first_of("\r\n") != std::string::npos) {
        die("Mesh height-source annotation must be a single line.");
    }
    if (!opt.printableMeshPath.empty() &&
        (!std::isfinite(opt.pixelScaleMm) || opt.pixelScaleMm <= 0 ||
         !std::isfinite(opt.printableThicknessMm) || opt.printableThicknessMm <= 0 ||
         !std::isfinite(opt.heightScale))) {
        die("Printable PLY requires positive finite mm/pixel and base thickness, and finite height scale.");
    }
    if (opt.printableFillHoles && !opt.printableMeshPath.empty()) {
        if (!opt.meshPath.empty()) {
            const PlyGridTopology openTopology = buildPlyGridTopology(
                height, geometryMask, opt.meshStep, false, progress);
            writePlyMesh(opt.meshPath, height, vertexColor, opt.heightScale, openTopology, progress);
        }
        const PrintableSurfaceFill printable = fillPrintableSurfaceHoles(
            height, geometryMask, normalMap, vertexColor, opt.heightSlopeCap, progress);
        writeImageChecked(fs::path(opt.outputDir) / "printable_fill_mask.png", printable.fillMask);
        PlyGridTopology topology = buildPlyGridTopology(
            printable.height, printable.mask, opt.meshStep, false, progress);
        preparePrintablePlyTopology(topology, printable.height, progress);
        writePrintablePlyMesh(opt.printableMeshPath, printable.height, printable.vertexColor,
            opt.heightScale, opt.pixelScaleMm, opt.printableThicknessMm, topology, progress, heightSource);
    } else {
        PlyGridTopology topology = buildPlyGridTopology(height, geometryMask, opt.meshStep, false, progress);
        if (!opt.meshPath.empty()) {
            writePlyMesh(opt.meshPath, height, vertexColor, opt.heightScale, topology, progress);
        }
        if (!opt.printableMeshPath.empty()) {
            preparePrintablePlyTopology(topology, height, progress);
            writePrintablePlyMesh(opt.printableMeshPath, height, vertexColor,
                opt.heightScale, opt.pixelScaleMm, opt.printableThicknessMm, topology, progress, heightSource);
        }
    }
}
