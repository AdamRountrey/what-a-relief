...
2. Determine lighting either by marking a highlight sphere, loading a previous `lights.csv` or `light_vectors.csv`, or using the uncalibrated unknown-lighting mode. Optionally rectify a fixed microscope working plane with a printable checkerboard calibration and estimate effective relative light/exposure gains for the current stack.
3. Convert the image stack to grayscale working intensities and solve a classical Lambertian normal field per pixel.
4. In robust calibrated mode, jointly fit normal and relative albedo with continuous pseudo-Huber/Cauchy-weighted least squares, then report model-relative shadows, highlights, clipping, and confidence-style diagnostics.
...
10. As a separate experimental choice, run an isolated Mitsuba differentiable-rendering backend that either proposes a smooth correction or, in Ultra, optimizes an absolute height field on a grid capped at 1024 pixels on its longer side and initialized from the classical result. It models spatially varying diffuse and glossy response and accepts a candidate only through training and withheld-light checks.
...
The optional Mitsuba inverse geometry stage is also not metrology. Preview, Standard, and High detail optimize a bilinear height correction; Ultra instead optimizes absolute Z with one parameter per fitting-grid pixel, capped at 1024 pixels on the longer side, with the classical height used as initialization and fallback rather than an additive output layer. If the input crop is larger, an accepted solution is interpolated back to the source grid. All modes retain fixed image-plane vertex coordinates and approximate the fixed microscope camera with a long-distance narrow-field perspective camera. Finite disk emitters replace delta point/directional sources for the projective shadow-boundary estimator: near-field use requires a measured positive LED diameter; directional use is an explicitly approximate distant disk with a default full angular diameter of 1 degree. Diffuse/glossy basis weights are periodically refitted from training lights while the BRDF basis shapes remain fixed. Full-precision classical photometric normals provide a confidence-weighted orientation constraint. The best training-loss-plus-normal-prior checkpoint must still pass separate withheld-light, normal-consistency, and displacement checks. A lower rendered-image loss or a withheld-light check does not establish physical accuracy, including on black glossy specimens. Separate inverse outputs preserve the classical baseline. See the [independent reconstruction test](docs/inverse-pipeline-test.md) for measured gains and regional limitations.

The [September 2026 review-fix ledger](docs/review-fixes-2026-09.md) records R1-R9, the verified CPU results, and the earlier backend installer. Version 0.2.2 includes the subsequent height-integration corrections and measured-normal constraint. The [independent curved-scene pipeline test](docs/inverse-pipeline-test.md) passes its global height-improvement criteria on CPU and CUDA, including a rotated-light capture on CUDA. Local height regressions remain, and the original photometric normals are still more accurate than the inverse normal product. These results do not qualify general reconstruction accuracy. Updating files in this repository does not update an installed copy; use the matching application and optional backend packages.
...
The executable target is `what-a-relief`; on Windows the direct build writes `build-vcpkg-direct\what-a-relief.exe`. The direct build and installer accept `-Version` (currently `0.2.16`); CMake accepts `-DWHAT_A_RELIEF_VERSION_STRING=0.2.16`. This version is recorded in run manifests. When packaging with `-SkipBuild`, use the same version as the already-built executable.
...
GitHub Actions runs the baseline checks before creating an installer or portable archive. A successful compile by itself is not considered a passing build. The optional real-Mitsuba CPU unittest compares finite-ring and approximate-directional moving-shadow AD against finite differences, and exercises a 24 x 24, eight-light, six-iteration tilt-recovery/truth-start fixture with an internal mask hole and spatial diffuse/glossy coefficients. These tests passed in the recorded eight-test LLVM 15.0.7 run, including normal-angle and height-RMSE checks. Derivative and recovery tests run in separate processes, matching the application's isolated probe/job lifetime. The startup probe checks for a finite, nonzero moving-shadow derivative, but does not replace finite-difference agreement or reconstruction tests. Exact metrics, evidence limits, and historical reference values are in [`docs/validation.md`](docs/validation.md); the durable release checklist is in [`docs/quality-roadmap.md`](docs/quality-roadmap.md).
...
The installer is written to `dist\what-a-relief-0.2.16-setup.exe` by default. It installs under `%LOCALAPPDATA%\Programs\what-a-relief`, creates a Start Menu shortcut, and registers an uninstall entry for the current user. It does not require administrator privileges.
...
The optional `dist\what-a-relief-0.2.16-mitsuba-backend-setup.exe` bundles private CPython 3.13.13, Mitsuba 3.8.0, a source-built patched Dr.Jit Core 1.3.1, NumPy 2.3.3, and official LLVM 15.0.7. Run it separately after installing the main application to place the backend under `%LOCALAPPDATA%\Programs\what-a-relief-mitsuba`; users do not install Python, pip, Conda, Anaconda, or LLVM separately. The patch prevents a native CUDA block-reduction overflow on very large differentiable workloads; its exact source diff and provenance marker are bundled. Packaging requires staged numerical and real CPU rendering tests to pass. The [earlier package record](docs/review-fixes-2026-09.md#backend-package) documents the runtime compatibility investigation; those historical hashes are not hashes of the current package. Extraction tests do not install or modify the user's existing application.
...
The setup window lets you choose the image set, output folder, lighting mode, optional previous `lights.csv` or `light_vectors.csv`, optional near-field ring-light geometry, image scale, optional specimen mask for height and mesh, normal solver, optional relief flattening, sRGB handling, height solver, height drift correction, height preview, PLY export, optional printable-surface hole filling, optional RTI export, experimental diagnostics, optional cast-shadow height refinement, optional experimental Mitsuba inverse refinement, optional experimental neural fusion, and the interactive relight viewer before computation starts. If you use calibrated sphere mode, choose any photograph and mark at least five well-spaced points on the sphere edge, or load a previous calibration CSV to skip sphere marking. For a previous near-field ring run, load the full `lights.csv` so the ring radius, height, effective LED diameter, and pixel scale are restored along with the light vectors. If its recorded image names do not match the new image set, the GUI can instead apply its rows in order after an explicit warning and confirmation. If you skip the sphere, the program uses an unknown-lighting solve and requires at least 4 images. Uncalibrated mode can also crop to the surface region so shiny fixtures, the calibration sphere, and background do not contaminate the solve. The specimen height mask is separate from the solve mask: it limits only `height.png`, `height.pfm`, `height_mask.png`, and optional PLY export. Mitsuba refinement and cast-shadow height refinement are alternative advanced geometry experiments; the GUI will not run both in one job.

### Microscope checkerboard and light balance

On the **Lighting** tab, **Make Target** writes an exact-size SVG checkerboard with settable square count and square size. Print it at **100% / Actual size**, disable printer scaling, and verify the included 10 mm line. Mount the print flat and fronto-parallel at the specimen working plane, keep the camera, focus, and optics fixed, and capture one registered image under every lighting position. **Calibrate** detects the grid, derives millimeters per rectified pixel, fits a smooth working-plane warp, measures unclipped white-minus-black cell contrast, and saves a reusable JSON file. The board should fill most of the field; calibration is rejected below 20% image coverage, above 0.75-pixel RMS corner error, above 2.5-pixel maximum corner error, or when too few cells have usable contrast.

This is a fixed-working-plane microscope correction, not a general pinhole-camera calibration. One fronto-parallel board cannot uniquely recover general camera intrinsics, and the correction must not be reused at a different resolution, focus, zoom, camera pose, or specimen plane. External masks used with it must already use rectified coordinates. Light directions still come from the sphere, a lights file, or fixed ring geometry.

Checkerboard brightness measurements are stored as a reference prior, not blindly applied to later specimens. Exposure can vary between image sets, and per-image exposure or response changes alter effective brightness. Enable **Estimate per-stack relative light balance** (automatically selected after microscope calibration) to estimate one effective gain per lighting image from diffuse specimen pixels. It requires at least five known light directions and real surface-normal diversity, compares two spatial partitions, and applies the correction only when leave-one-light-out prediction improves by at least 2%, spatial log-gain RMS is at most 0.10, and no gain exceeds the supported twofold range. Rejection retains equal gains. The same validated estimation is available for non-microscope calibrated sets.

Command-line equivalents are:

```powershell
what-a-relief.exe --make-calibration-target checkerboard.svg --calibration-grid 12 9 2 --calibration-page 210 297
what-a-relief.exe --create-microscope-calibration microscope.json --calibration-grid 12 9 2 `
  --image checker_light_01.tif --image checker_light_02.tif --image checker_light_03.tif
what-a-relief.exe --microscope-calibration microscope.json --estimate-light-gains `
  --image specimen_light_01.tif --image specimen_light_02.tif --image specimen_light_03.tif `
  --image specimen_light_04.tif --image specimen_light_05.tif --lights-file lights.csv --out result
```
...
The image set opens in a sphere picker window. Use Page Up/Page Down (or `[`/`]`) to choose whichever photograph shows the clearest sphere edge; changing photographs clears any in-progress points. Use the mouse wheel or `+`/`-` to zoom, right-drag or use `WASD`/arrow keys to pan, then click at least five well-spaced points around the sphere edge in that one photograph. The circle turns green only after its point coverage and fit residuals pass. Press Enter or Space to accept, Backspace to remove the last point, `R` to reset the points, `0` to fit the image, or Esc to cancel. The sphere may be away from the frame center, but its complete outline must remain visible.
...
- `microscope_calibration.json`: preserved copy of the selected fixed-working-plane calibration, including checkerboard geometry, cubic output-to-source warp, fit residuals, physical pixel scale, and reference light gains. Written only when this calibration is used.
...
- `lights.csv`: selected sphere geometry and source image, circle-fit evidence, estimated highlight points, light vectors, thresholds, peaks, and per-image highlight-quality evidence (connected area, competing components, clipping, compactness, centroid uncertainty, and radial position). In uncalibrated mode this file records that no calibrated light vectors were used.
...
- `shadow_refinement/`: written only with `--specular-diagnostics`; contains full-resolution per-light regularized observed cast-shadow masks, evidence confidence, and predicted masks and probabilities before and after an evaluated correction. Routine shadow refinement does not create this debugging folder. The manifest's `diagnostics.shadow_height_refinement` object records whether refinement was applied, the decision, mismatch change, correction size, and safety-gate measurements.
- `inverse/`: guarded experimental Mitsuba outputs and `result.json`. On rejection, `inverse_height.pfm`, the inverse geometry previews/mesh, and material maps retain the baseline; root `height_correction.pfm/png` represents a zero correction. Accepted runs deliver the refinement here without an extra candidate copy.
- `inverse/render_before.png` and `inverse/render_after.png`: mean training renders using a shared display range. The after image always describes the selected candidate, even on rejection; it is not a render of the guarded baseline fallback.
- `inverse/unvalidated_candidate/`: finite rejected geometry, warning-bearing `candidate_surface.ply`, `candidate.json` provenance with `accepted:false`, and an offline review report. Nonfinite candidate geometry is not exported. See [Rejected Candidate Review](#rejected-candidate-review) and the [complete file list](tools/mitsuba_backend/README.md#rejected-candidate-outputs-and-review).
...
- `printable_fill_mask.png`: written when printable smart filling is enabled; white pixels identify enclosed gaps synthesized before printable mesh sampling and component selection. This mask does not alter the height products or open inspection mesh.
...
In `inverse/unvalidated_candidate/review.html`, choose baseline only, candidate only, or both side by side, and select height, height-derived RGB or X normals, hillshade, or mean training render. Height comparison uses `review_baseline_height.png` and `review_candidate_height.png` with one shared range; the ordinary height PNGs are visualization products, not quantitative height data. The report includes the rejection reason and baseline/candidate losses, with a link to the full validation result. Check the failed-validation acknowledgement to reveal explicit candidate file links. This is a warning acknowledgement, not an acceptance or promotion control.

Candidate height and correction PFMs use image-pixel height units. Candidate normals come from the candidate height gradient, not the original photometric normal map or ground truth. Mesh XY uses pixels and Z includes `--height-scale`; the candidate PLY is an inspection surface, not a watertight printable solid. Keep `candidate.json` with exported candidate files. Retention and review do not change the solver or acceptance thresholds and make no numerical accuracy claim. Rerunning into the same output folder replaces the old `inverse/` folder, including its candidate and report; this is not an archive.

### Optional Robust Diagnostics

Robust summary images are off by default. Enable **Write additional robust diagnostic images (summary and per-light)** on the Advanced tab, or pass `--specular-diagnostics`, to write the following maps. This changes diagnostic export, not the robust normal solve. A basic run writes nine PNGs: RGB/X/Y/Z normals, upper-left hillshade, albedo, residual, valid mask, and liquid metal. Selected height, mesh, neural, RTI, and advanced-refinement products are additional. Reusing an output folder removes stale generated diagnostics from an earlier run.
...
- `--input-response auto|linear|srgb`: input transfer response. Default `auto` reads each image's supported color metadata and reports any assumptions. The Processing tab has the same choices.
- `--srgb` / `--linear`: explicit stack-wide overrides, ignoring color metadata. `--srgb` retains its previous meaning.
- `--solver standard|robust`: calibrated normal solver. `robust` is the default and uses continuous pseudo-Huber initialization and adaptive Cauchy-weighted refinement. No spatial normal smoothing is applied.
- `--high-outlier-threshold value`: legacy normalized probable-clipping cutoff for solver callers without raw-channel headroom information. Normal GUI/CLI loading now supplies headroom information and uses the actual integer-container white limit instead. Default: `0.98`.
...
- `--specular-diagnostics`: with the calibrated robust solver, opt in to all nine robust summary maps, the aggregate specular cue, and per-light shadow, highlight, and saturation masks. Off by default.
...
- `--shadow-reference-z-mm value`: shared by either near-field shadow or Mitsuba refinement, despite the historical name. Assign the inferred flat-surface/percentile height datum this physical Z in the same millimeter coordinates as ring height; it must be below the lights. Default: `0`. It does not change the classical normal solver's `z = 0` approximation.
- `--shadow-led-diameter-mm value`: effective circular emitting diameter in millimeters for either near-field refinement. Shadow-only refinement allows `0` for point-source visibility; near-field Mitsuba requires a measured value greater than `0`. Default: `0`, so a near-field inverse run needs explicit input.
...
- `--mitsuba-quality preview|standard|research|ultra`: work budget and geometry mode, not an accuracy rating. Default: `standard`; see the [preset table](tools/mitsuba_backend/README.md#quality-presets). The GUI offers Preview, Standard, and High detail (`research`) only; Ultra remains available through the CLI for controlled experiments and for compatibility with existing project files. Preview/standard/research use 64/128/256-pixel longest-side grids and 12/24/50 iterations, then add the upsampled correction to the full-resolution baseline. Ultra uses 75 iterations and one absolute-height control per fitting-grid pixel on a grid capped at 1024 pixels on its longer side; the baseline initializes it but is not added back. Its stochastic AD sampling adapts from 32 down to 1 spp so a pass contains at most about eight million pixel samples, while fixed-seed validation remains at 256 spp. Its Adam step is also capped at 2% of the normalized fitting-grid pixel pitch to prevent a resolution-dependent high-curvature first update. Effective values are recorded in `result.json`. Ultra can still require hours or exhaust memory. A 1024-pixel solve does not validate reconstructed detail at that scale.
...
Use TIFF or other supported, minimally processed images when possible. Develop camera raw files externally with consistent settings. JPEGs can work, but compression artifacts, clipping, and camera tone processing can reduce accuracy even after sRGB decoding. The program converts color images to luminance before solving; it does not use color channels independently.

Input response now defaults to **Auto**. It recognizes Exif sRGB tags in JPEG/classic TIFF, supported PNG color chunks, and supported sRGB or linear-sRGB matrix/TRC ICC profiles. Untagged JPEGs are **assumed sRGB**; other untagged inputs are **assumed linear**, preserving the usual scientific-image interpretation. The GUI reports those assumptions, and `run_manifest.json` records the decision and evidence for every input in image order. Unsupported profiles, ambiguous gamma tags, malformed metadata, and BigTIFF require conversion or an explicit response override; Auto is not a general ICC color converter. Mixed tagged linear/sRGB stacks are decoded per image. Choose Linear or sRGB explicitly when the metadata does not describe your actual pixels.

sRGB decoding uses the standard piecewise function on each color channel before luminance conversion, once only. Normal solving and RTI use the same decisions; inverse rendering receives already-linear observations. The normal stack is normalized by one common multiplicative scale, preserving light-to-light ratios. Color tags describe encoding, not radiometric calibration: decoding cannot undo camera tone mapping, sharpening, clipping, JPEG loss, or automatic exposure. For scientific imaging, a linear, dark-corrected, flat-field-corrected, consistently exposed stack remains preferable. See [input response details](docs/algorithm.md#input-response).

The calibrated normal solve is diffuse Lambertian. At each pixel, the standard solver omits observations below `--shadow-threshold` and needs at least three usable observations. The robust solver jointly fits `g = albedo * normal` using a convex pseudo-Huber start and an adaptive Cauchy refinement. Each phase allows up to 80 IRLS updates with early convergence and objective-descent checks; the final phase's residual scale is not capped or changed between its updates. There is no discrete hypothesis winner, independent albedo reset, or spatial normal smoothing. Raw integer RGB headroom smoothly reduces observation weights near clipping, ignoring alpha opacity; exact clipping is still excluded. A narrow low-signal taper also admits measurements gradually above the dark cutoff. The signal reference and bounded residual-scale estimate use the same input reliability, so barely trusted observations cannot abruptly receive a full scale vote. Model-relative shadow/highlight labels are computed after fitting and do not change the estimated normal. A locally weighted light matrix with condition number above 100, or fewer than three effective observations, produces an explicit unsupported pixel. Three effective observations provide little outlier redundancy. The manifest records the estimator identifier, mean iteration count, and nonconverged fraction. Robust fitting is not guaranteed to outperform least squares: broad gloss, interreflection, wrong response interpretation, unequal light brightness, and misregistration can bias either estimator. See [validation](docs/validation.md#input-response-and-robust-stability), the [implementation and publication record](docs/robust-estimator-update-2026-09-09.md), and the [threshold-stability update](docs/threshold-stability-2026-09-10.md).
...
`--specular-diagnostics` is experimental and requires the calibrated robust solver. It enables robust summary images, an aggregate cue, and auditable per-light classifications. Highlights require a significant positive model-relative residual and illumination-view half-vector agreement; shadows require attached-shadow evidence or a substantial intensity deficit. A conditional noise-propagation estimate makes shadow classification more conservative when the fitted light geometry is uncertain. These are heuristic fitted-model labels, not calibrated probabilities or semantic ground truth. They do not replace the normal, recover lighting, or make broad non-Lambertian reconstruction reliable. Spatial outlier-field regularization is not implemented in the normal solver.
...
After a GUI run, the completion status says whether shadow refinement was applied, gives the balanced-mismatch change and correction RMS when accepted, or gives the rejection reason and confirms that the original height was retained. The same values remain available in `run_manifest.json` under `diagnostics.shadow_height_refinement`.

The optional Mitsuba inverse stage receives the same post-radiometric luminance observations used by the classical solve. what-a-relief encodes them temporarily as globally normalized 16-bit linear grayscale PNGs, so the isolated renderer never needs to understand the original TIFF, PNG, JPEG, or microscope-vendor container. This preserves light-to-light ratios with at most one 16-bit code of transfer quantization and avoids a second color or gamma interpretation. Schema `2` also transfers per-light validity masks derived from the original finite, nonnegative samples and decoder clipping masks, before area downsampling can dilute clipping. Material fitting and height loss use the same observation weights; legitimate shadow evidence is not discarded simply for being dark. The job records source paths and transfer metadata; temporary observations and validity files are removed after processing. Container-maximum clipping detection is not a calibrated sensor saturation model.

The Python worker decodes and resamples observations and validity one light at a time. Preview, Standard, and High detail retain coarse stacks; Ultra uses at most a 1024-pixel-long-side crop. It reuses three basis scenes across lights and accumulates each training light's gradient before one Adam update per iteration. Ultra uses an identity height map instead of dense interpolation tables, streams one observation/validity tensor at a time, evaluates height priors with exact host-side gradients, and replays material bases so only one renderer AD graph is retained at once. Its per-pass sampling is capped by an eight-million-pixel-sample memory guard. Differentiable loss sums are hierarchical, retaining every optimization-grid term. The bundled Dr.Jit Core additionally patches its native CUDA block and prefix reductions so renderer-internal reductions above 2^31 entries take the existing 1024-wide large-block path instead of overflowing their uint32 next-power-of-two calculation. Ultra CUDA verifies the patch marker against the loaded DLL before starting. Source-resolution observations and the C++ input stack still occupy memory; this is not an out-of-core pipeline or a measured speedup claim. Production inverse scenes retain both primary and indirect silhouette sampling. The three refinement modes erode their fitting mask by one pixel; Ultra instead includes every fitting-grid mask vertex that belongs to a rendered triangle, including boundary pixels. Pixels outside the geometry mask, nonfinite samples, and clipped/saturated equality measurements remain excluded; valid dark pixels remain included. Fixed XY support still limits outline evidence, and enabled primary derivatives do not turn this into free-outline reconstruction or validate arbitrary camera occlusion on real specimens.

Shadow and inverse refinement were already alternative modes, not consecutive stages. Both now use the same median sufficiently flat height datum, with a tenth-percentile fallback, and assign it `--shadow-reference-z-mm` for near-field geometry. This is an explicit coordinate convention, not an absolute-height measurement. Shadow-only runs without `--specular-diagnostics` retain one 16-bit class/confidence observation per light and pixel (3 class bits, 13 confidence bits), regularize one light at a time, and keep per-light audits at coarse resolution. Full diagnostics retain the larger full-resolution products; the reported observation-storage estimate excludes input images, geometry, and temporary work buffers.
...
PLY meshes are written as buffered binary little-endian files. Printable boundaries use compact grid bookkeeping rather than retaining every triangle edge. The regular `surface.ply` uses image pixel coordinates for `x` and `y` and the relative height preview for `z`. Use `--height-scale` for visual exaggeration and `--mesh-step` to keep large captures from producing enormous files; step `2` samples one quarter as many grid vertices as step `1`. The printable PLY export writes one closed solid with duplicated bottom vertices, reversed bottom faces, and side faces along every open boundary edge. It always retains the largest edge-connected surface after mesh sampling, ranked by projected XY area, removes unused vertices, and trims ambiguous point contacts. Disconnected pieces are discarded, not joined by invented bridges. Use a smaller mesh step if thin features become disconnected. This component selection affects only the printable PLY, not the regular PLY, height map, or normals.

Printable export requires XY scale so coordinates and base thickness are in millimeters. Optional smart filling identifies invalid components fully enclosed by the geometry domain, keeps every measured height fixed, extends nearby height gradients into each gap, and solves a slope-guided Poisson interpolation there. Normal-derived slopes are used only when a local height derivative is unavailable. Exterior and boundary-connected exclusions are preserved. This is explicitly synthesized geometry for printing, not recovered measurement, and it affects neither `height.pfm` nor `surface.ply`. The fill mask records synthesized pixels before mesh sampling and component selection. Image preparation and RTI coefficient or tile generation use OpenCV's worker pool, while PNG, JPEG, and TIFF encoding remains delegated to OpenCV's native codecs with unchanged quality settings.
...
- Paul Holland and Roy Welsch's work on IRLS and Yvain Queau and colleagues' CVPR 2017 work on non-convex photometric stereo inform the continuous robust fitting. This is an independent pointwise adaptation, not Queau's full joint depth/lighting solver. The earlier minimal-subset consensus implementation has been replaced. See the [publication-to-implementation record](docs/robust-estimator-update-2026-09-09.md).
...
- Wenzel Jakob and contributors developed Mitsuba 3 and Dr.Jit. Mitsuba 3.8.0 generated the committed mixed-material validation corpus, including GGX rough-plastic reflection, cast shadows, and reference normal AOVs. It is not part of the standard application package or baseline CTest tier; the pinned renderer is distributed in the optional inverse-rendering add-on. See `tests\fixtures\mitsuba\README.md` and `THIRD_PARTY_NOTICES.md`.
- Ziyi Zhang, Nicolas Roussel, and Wenzel Jakob developed [Projective Sampling for Differentiable Rendering of Geometry (2023)](https://rgl.epfl.ch/publications/Zhang2023Projective). Its visibility-derivative method does not validate this application's shape estimates. The [Mitsuba maintainer's delta-source explanation](https://github.com/mitsuba-renderer/mitsuba3/issues/1443#issuecomment-2577492612) and [area-emitter documentation](https://mitsuba.readthedocs.io/en/stable/src/generated/plugins_emitters.html#area-light-area) motivate the finite-source inverse model; source size changes the forward shadows and must be physically justified or declared approximate.
