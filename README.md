...
- `shadow_refinement/`: per-light regularized observed cast-shadow masks, evidence confidence, and predicted masks and probabilities before and after an evaluated correction. These per-light audits are coarse by default; `--specular-diagnostics` retains full-resolution products. The manifest's `diagnostics.shadow_height_refinement.decision` is authoritative when evidence was insufficient before every audit could be computed.
...
The Python worker decodes and area-reduces observations and validity one light at a time, retaining the coarse stack rather than another full-resolution stack. It reuses three basis scenes across lights and accumulates each training light's gradient before one Adam update per iteration. Full-resolution geometry and the C++ input stack still occupy memory; this is not an out-of-core pipeline or a measured speedup claim. Production inverse scenes retain both primary and indirect silhouette sampling. Fixed XY support and an eroded fitting mask still limit outline evidence. Enabled primary derivatives do not turn this into free-outline reconstruction or validate arbitrary camera occlusion on real specimens.
